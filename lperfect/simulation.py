    ...
    for ctx in contexts:
        ...
        if out_nc:
            volgrid = local_volgrid_from_particles_slab(
                ...
                r0=ctx.r0,
                r1=ctx.r1,
                ncols=ctx.dom.dem.shape[1],
                shared_cfg=shared_cfg,
            )
            if size > 1:
                vol_full = gather_field_slab_to_rank0(comm, volgrid, ctx.dom.dem.shape[0], ctx.dom.dem.shape[1])
            ...
            if rank == 0:
                ...
                write_results_netcdf_rank0(
                    ...
                    flood_depth,
                    risk_field,
                    time_hours=datetime_to_hours_since_1900(start_datetime + timedelta(seconds=float(final_elapsed_s))),
                    mode="w",
                )
        # Quality log.
        ...
        if rank == 0:
            ...
            logger.info(
                "[%s] quality: rain=%.3e runoff=%.3e outflow=%.3e stored=%.3e mass_err=%.3e",
                ctx.label,
                report["rain_m3"],
                report["runoff_m3"],
                report["outflow_m3"],
